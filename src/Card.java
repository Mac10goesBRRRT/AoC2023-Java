...
public class Card implements Comparable<Card>{
    ...
    public int calculateScore(String value) {
        ...
        //System.out.println(retValue + " " + sortedEntries.get(1).getValue() + " " + numJokers);
        //TODO: viele Sachen mit 1 nem Joker und nur 1er passenden Karte landen zu weit hinten
        ...
        if(retValue == 4 || (retValue == 3 && numJokers == 1) || numJokers == 3 || checkQuad(sortedEntries) ) //4, can be upgraded to 5 with any joker, and we simply add the number of Jokers (1 or none)
             return 5; //simply add Jokers, does not change anything else but make a //TODO hier passt was noch nicht
        if((retValue==3 && sortedEntries.get(1).getValue()==2) || (retValue == 2 && numJokers == 1 && sortedEntries.get(1).getValue()==2)) //Full House, if we have a Triple and a Pair. Or we have  2 + 2 (so pair of 2 and 2 jokers
            return 4;
        else if(retValue==3 || numJokers == 2 || (retValue==2 && numJokers == 1)) //Triple, or 2 Jokers and 1 random card
            return 3;
        else if((retValue==2 && sortedEntries.get(1).getValue()==2)) //2 Pairs or 1 Pair and 2 jokers. !!! 1 Pair and 1 joker makes a Triple
            return 2;
